...
if ! detect_display && [[ -z "${WAYLAND_DISPLAY:-}" ]]; then
  echo "[aurora-room] No display found -- room requires a graphical session."
  exit 0
fi
...
if [[ -z "$PYTHON_BIN" ]]; then
  echo "[aurora-room] Missing GUI dependencies. Aurora Room requires tkinter + PIL.ImageTk."
  echo "[aurora-room] Recommended fix: use the project .venv or install a Pillow build with Tk support."
  exit 0
fi

echo "[aurora-room] Launching with $PYTHON_BIN"
if [[ -n "${DISPLAY:-}" ]]; then
  echo "[aurora-room] DISPLAY=$DISPLAY"
fi
if [[ -n "${XAUTHORITY:-}" ]]; then
  echo "[aurora-room] XAUTHORITY=$XAUTHORITY"
fi

if ! wait_for_tk_display "$PYTHON_BIN" 20; then
  echo "[aurora-room] Tk still cannot reach the display. Exiting cleanly."
  exit 0
fi

exec "$PYTHON_BIN" -u aurora_room.py
