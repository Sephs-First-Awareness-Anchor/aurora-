...
class RuntimeUnderstandingContract:
    ...
    def _derive_meaning_state(
        ...
        noncomp_input = dict((understood or {}).get("noncomp_input") or {})
        noncomp_state = dict((understood or {}).get("noncomp_state") or noncomp_input.get("noncomp_state") or {})
        noncomp_manifold = dict(noncomp_state.get("manifold") or {})
        input_constraint = str((understood or {}).get("constraint", "") or "").strip()
        dominant_target = str(
            noncomp_state.get("dominant_target", "")
            or noncomp_manifold.get("dominant_target", "")
            or input_constraint
            or ""
        ).strip()
        basis_channel = str(
            noncomp_state.get("basis_channel", "")
            or noncomp_manifold.get("basis_channel", "")
            or ""
        ).strip()
        noncomp_compaction = _clip01(noncomp_state.get("compaction", 0.0), 0.0) if noncomp_state else 0.0
        noncomp_salience = _clip01(noncomp_state.get("salience", 0.0), 0.0) if noncomp_state else 0.0
        noncomp_entropy = _clip01(
            noncomp_state.get("manifold_entropy", noncomp_state.get("entropy", 0.0)) if noncomp_state else 0.0,
            0.0,
        )
        noncomp_target_weight = _clip01(
            noncomp_state.get("dominant_target_weight", noncomp_manifold.get("dominant_target_weight", 0.0)) if noncomp_state else 0.0,
            0.0,
        )
        manifold_pressure = _clip01(
            (1.0 - noncomp_compaction) * 0.55
            + (1.0 - noncomp_salience) * 0.15
            + noncomp_entropy * 0.20
            + (1.0 - noncomp_target_weight) * 0.10,
            0.5,
        )
        ...
        if noncomp_state:
            meaning_delta = round(
                _clip01(
                    max(
                        meaning_delta,
                        (0.45 * noncomp_compaction)
                        + (0.35 * noncomp_salience)
                        + (0.20 * noncomp_target_weight),
                    ),
                    0.5,
                ),
                4,
            ...
            semantic_pressure = round(
                _clip01(
                    (semantic_pressure * (1.0 - (0.20 * noncomp_compaction))) + (manifold_pressure * 0.35),
                    0.5,
                ),
                4,
            )
            active_meaning["constraint_translation"] = str(
                noncomp_state.get("semantic_translation", "")
                or noncomp_state.get("manifold_translation", "")
                or ""
            ).strip()
            if dominant_target:
                active_meaning["dominant_noncomp_target"] = dominant_target
            active_meaning["noncomp_channel_count"] = int(noncomp_state.get("channel_count", 25) or 25)
            active_meaning["noncomp_manifold_slots"] = int(
                noncomp_state.get("manifold_slot_count", 125) or 125
            )
            if basis_channel:
                active_frame["noncomp_basis_channel"] = basis_channel
            if noncomp_manifold:
                active_frame["noncomp_manifold"] = {
                    "dominant_target": dominant_target,
                    "slot_count": int(noncomp_manifold.get("slot_count", 125) or 125),
                    "translation": str(noncomp_manifold.get("translation", "") or "").strip(),
                }
    ...
    def _evaluate_previous_accuracy(
        ...
        noncomp_state = dict(
            ...
            or understood.get("noncomp_input")
            or {}
        )
        ...
        is_question = bool(
            understood.get("is_question")
            or str(understood.get("query_type", "") or "").strip() in {"question", "clarification", "definition"}
            or noncomp_state.get("needs_remedy")
            or self._looks_like_question(user_text)
        )
        is_clarification = bool(
            understood.get("is_clarification")
            or understood.get("is_callback")
            or noncomp_state.get("remedy") == "clarify"
            or any(marker in text_low for marker in clarification_markers)
        )
        is_callback = bool(understood.get("is_callback") or noncomp_state.get("remedy") == "lookup")
        is_correction = bool(
            understood.get("is_contradiction")
            or understood.get("is_correction")
            or noncomp_state.get("needs_remedy")
            or any(marker in text_low for marker in correction_markers)
        )
        ...
        if expected_affect and expected_affect not in ("neutral", "") and not is_correction:
            understood_affect = str(understood.get("tone", "") or "").lower()
    ...
    def _derive_observation(self, user_text: str, understood: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "correction": bool(
                understood.get("is_contradiction")
                or understood.get("is_correction")
                or noncomp_state.get("remedy") in {"clarify", "lookup"}
                or noncomp_state.get("needs_remedy")
    ...
    def _derive_policy_state(
        ...
        *,
        ...
        elif "look it up" in text_low or noncomp_state.get("remedy") == "lookup":
            action_type = "lookup_offer"
            expected = "confirmation"
        elif "contradict" in text_low or "conflict" in text_low or noncomp_state.get("remedy") == "clarify":
        ...
        expected_topic = str(
            noncomp_state.get("anchor")
            or noncomp_state.get("dominant_target")
            or self.state.get("M", {}).get("active_topic", "")
            or ""
        ).strip()
    ...
    def audit_candidate_response(
        ...
        *,
        ...
        noncomp_state = dict(
            dict(understood or {}).get("noncomp_state")
            or dict(understood or {}).get("noncomp_input")
            or {}
        )
        callback_like = bool(
