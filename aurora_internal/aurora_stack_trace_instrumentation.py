...
from aurora_constraint_engine import ConstraintVector as _ConstraintVector
...
def _trace_constraint_profile(
    ...
    return _ConstraintVector(
        X=max(1e-9, float(axes.get("X", 0.20))),
        T=float(axes.get("T", 0.25)),
        N=float(axes.get("N", 0.15)),
        B=float(axes.get("B", 0.20)),
        A=float(axes.get("A", 0.20)),
    )
