...
class ManualCodeLineageAssimilator:
    ...
    def assimilate(self, systems: Any, *, source: str = "runtime.boot", force: bool = False) -> List[Dict[str, Any]]:
        ...
        for change in changes:
            ...
            chamber = _system_get(systems, "code_evolution_chamber")
            if chamber is not None and hasattr(chamber, "snapshot"):
                try:
                    snapshot = chamber.snapshot(target_files=payload["target_files"])
                    result["code_pressure_snapshot"] = snapshot.to_dict() if hasattr(snapshot, "to_dict") else dict(snapshot or {})
                except Exception:
                    pass
