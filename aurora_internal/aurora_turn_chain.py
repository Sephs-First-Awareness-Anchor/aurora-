...
class TurnUnderstandingState:
    ...
    # CBU profiles for each TurnChain stage (CBU directive Step 8)
    # Registered with CBURegistry on turn init; updated on Surface tick.
    # Information→X, Belief→B, Purpose→A, Meaning→N, Understanding→T (per directive §13 Step 8)
    cbu_information:   Any = field(default=None, repr=False, compare=False)
    cbu_belief:        Any = field(default=None, repr=False, compare=False)
    cbu_purpose:       Any = field(default=None, repr=False, compare=False)
    cbu_meaning:       Any = field(default=None, repr=False, compare=False)
    cbu_understanding: Any = field(default=None, repr=False, compare=False)
    ...
    def __post_init__(self):
        """
        Build and register CBU profiles for the five TurnChain stages.
        Per CBU directive §13 Step 8:
          Information→X_weight, Belief→B_weight, Purpose→A_weight,
          Meaning→N_weight, Understanding→T_weight.
        Uses per-turn unique IDs so Surface tick updates only this turn's CBUs.
        """
