...
# N is neutral in the leverage scalar, but it still needs a small mediator
# correction when the energy layer drifts away from X/T support.
_N_MEDIATOR_BIAS_RATIO: float = 0.40
...
class LeverageBiasEngine:
    ...
    def compute_nudges(
        ...
        n_support_bias = self._n_support_bias(magnitudes)

        # Build per-constraint nudges
        nudges: Dict[Constraint, PhaseNudge] = {}
        for c in (Constraint.X, Constraint.T, Constraint.N, Constraint.B, Constraint.A):
            delta = self._nudge_for_constraint(c, scalar, bias_magnitude, n_support_bias)
    ...
    def _nudge_for_constraint(
        ...
        introduce new rules.
        """
        if c is Constraint.N and n_support_bias > 1e-9:
            # N remains scalar-neutral, but when it is not supported by X/T it
            # becomes slightly easier to rebalance instead of staying inert.
            return -n_support_bias + self._dither()
    ...
    def _n_support_bias(self, magnitudes: Dict[Constraint, float]) -> float:
        """
        Small, opaque N mediator bias derived from X/T support mismatch.

        This does not make N part of the leverage scalar. It only prevents the
        energy layer from being operationally frozen while existence/time are
        asking for a different amount of support.
        """
        x = float(magnitudes.get(Constraint.X, 0.0) or 0.0)
        t = float(magnitudes.get(Constraint.T, 0.0) or 0.0)
        n = float(magnitudes.get(Constraint.N, 0.0) or 0.0)
        x_budget = REGISTRY.cost(Constraint.X).baseline_budget
        t_budget = REGISTRY.cost(Constraint.T).baseline_budget
        expected_n = ((x * x_budget) + (t * t_budget)) / max(1e-9, x_budget + t_budget)
        mismatch = abs(n - expected_n)
        if mismatch <= 1e-6:
            return 0.0
        scaled = mismatch / max(1.0, expected_n, n)
        return max(0.0, min(_MAX_BIAS * _N_MEDIATOR_BIAS_RATIO, _MAX_BIAS * scaled * 0.25))
...
def verify_leverage_scalar() -> Dict[str, object]:
    ...
    Checks:
        ...
        6.  N remains scalar-neutral but rebalances when X/T support mismatches
    ...
    # 6. N remains scalar-neutral, but X/T support mismatch makes it labile
    # enough to rebalance instead of freezing the energy layer.
    check(
        "N mediator becomes more labile when X/T support is unmatched",
        nudges_over[Constraint.N].flip_threshold_delta < 0.0,
