...
class DreamCurriculumQueue:
    ...
    def constraint_profile(self) -> _ConstraintVector:
        return _ConstraintVector(
            X=max(1e-9, min(1.0, 0.18 + self.completed_count / 25.0)),
            T=min(1.0, 0.28 + self.queue_length / 25.0),
            N=1.0 if self.current_pack_id else 0.16,
            B=min(1.0, 0.18 + sum(1 for r in self._completed.values() if r.requeue_recommended) / 10.0),
            A=min(1.0, 0.20 + sum(1 for r in self._completed.values() if r.improvement_detected) / 10.0),
        )
