...
def _derive_signature(weights: Dict[str, float], include_weighting: bool = True) -> str:
...
class DreamEpisodePack:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "DreamEpisodePack":
        return cls(
            ...
            constraint_signature=d.get("constraint_signature", "X"),
...
class ConversationEpisodeCompiler:
    ...
    def compile_from_json(
        ...
        # Pack 1: target the strongest weakness band
        if len(scored_payloads) >= self.PACK_SIZE:
            selected = _select_weakness_targeted(
                scored_payloads, top_weaknesses[:5], self.PACK_SIZE
            )
            pack = self._build_pack(selected, "weakness_targeted", top_weaknesses[:5])
            packs.append(pack)

        # Pack 2: target the next weakness band
        if len(scored_payloads) >= self.PACK_SIZE * 2:
            selected = _select_weakness_targeted(
                scored_payloads, top_weaknesses[5:10], self.PACK_SIZE
            )
            pack = self._build_pack(selected, "weakness_targeted", top_weaknesses[5:10])
            packs.append(pack)
    ...
    def _build_pack(
        ...
        # Rebase rubric pressure into the five-constraint manifold.
        axis_totals = {axis: 0.0 for axis in ("X", "T", "N", "B", "A")}
        axis_counts = {axis: 0 for axis in ("X", "T", "N", "B", "A")}
        for dim, score in rubric_profile.items():
            axis = _DIM_TO_AXIS.get(dim)
            if not axis:
                continue
            axis_totals[axis] += float(score)
            axis_counts[axis] += 1
        axis_weights = {
            axis: round(
                axis_totals[axis] / max(axis_counts[axis], 1),
                4,
            )
            for axis in axis_totals
        }
        constraint_signature = _derive_signature(axis_weights)
        dominant_ax = max(axis_weights, key=axis_weights.__getitem__) if axis_weights else "X"
        runtime_regime = {
            "dominant_axis": dominant_ax,
            "governor_weight": _GovernorWeights.AS_DICT.get(dominant_ax, 0.0),
            "axes": axis_weights,
        }
        ...
        weighted_form = runtime_regime.get("weighted_form") or {}
        dominant_axis = str(weighted_form.get("dominant_axis", "X"))
        modifiers["dominant_axis_weight"] = axis_weights.get(dominant_axis, 0.0)
        modifiers["weakest_axis_weight"] = min(axis_weights.values()) if axis_weights else 0.0
    ...
    def _save_packs(self, packs: List[DreamEpisodePack]):
        ...
        for pack in packs:
            ...
            manifest.append({
                ...
                "constraint_signature": pack.constraint_signature,
                "runtime_regime": dict(pack.runtime_regime),
                "language_projection": dict(pack.language_projection),
