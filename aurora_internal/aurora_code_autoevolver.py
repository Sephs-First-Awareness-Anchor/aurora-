...
class CodeAutoEvolver:
    def __init__(self, repo_root: str):
        ...
        self._source_tree_cache: Dict[str, Any] = {}
    ...
    def _surface_method_name(self, op_id: str, kind: str) -> str:
        base = str(op_id or "")
        # Strip generated lineage namespaces/prefixes until the underlying target
        # name is exposed.  Without the loop, repeated latent promotions can
        # generate reflect_evolved_reflect_* names that no longer match existing
        # runtime bindings.
        while base.startswith("evolved."):
            base = base[len("evolved."):]
        changed = True
        prefixes = (
            "reflect_evolved_",
            "latent_surface_",
    ...
    def _surface_alias_key(self, method_name: str) -> str:
        base = str(method_name or "").strip()
        while base.startswith("evolved."):
            base = base[len("evolved."):]
        prefixes = (
            "reflect_evolved_",
        ...
        while changed:
            ...
            for prefix in prefixes:
                if base.startswith(prefix):
                    ...
                    changed = True
                    break
        return self._sanitize_identifier(base, prefix="surface")

    def _active_surface_caller_files(self) -> List[str]:
        relpaths = [
            "aurora.py",
        ...
        return [self._abs_path(path) for path in relpaths]

    def _legacy_surface_bindings(self) -> List[str]:
        """
        Return reflect_* names that active native wrapper call sites still ask
        AuroraEvolvedSurfaceEngine for.  These names are compatibility promises:
        if a later generation chooses a different canonical surface name, the
        old binding should degrade through an alias instead of forcing rollback.
        """
        pattern = re.compile(
            r"getattr\s*\(\s*[^,\n]+,\s*'(?P<name>reflect_[A-Za-z_][A-Za-z0-9_]*)'\s*\)"
        )
        names: set[str] = set()
        for path in self._active_surface_caller_files():
            if not os.path.exists(path):
                continue
            try:
    ...
    def _select_reflection_candidates(self, operations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        feedback_cache: Dict[Tuple[str, str], Dict[str, float]] = {}
        ...
        for row in (operations or []):
            ...
            feedback_key = (module_name, rewrite_bias)
            if feedback_key not in feedback_cache:
                feedback_cache[feedback_key] = self._feedback_pressure(module_name, rewrite_bias, artifacts)
            feedback = feedback_cache[feedback_key]
            impact = float(summary.get("system_impact_score", 0.0) or 0.0)
            ...
            if xlinks < 1 and slot_pressure < 0.15 and not (
                ...
                selection_penalty += 0.30
            score = self._surface_score(row) + gap * 1.5 + min(0.15, xlinks * 0.01) + gpressure + fb_pressure + slot_pressure - selection_penalty
            row = dict(row)
            row["selection_penalty"] = round(float(selection_penalty), 4)
            row["selection_score"] = round(float(score), 6)
            kind = str(row.get("kind", "") or "")
    ...
    def _module_source_path(self, module_name: str) -> str:
        rel = str(module_name or "").strip().replace(".", "/")
        if not rel:
            return ""
        return self._abs_path(f"{rel}.py")

    def _module_ast(self, module_name: str) -> Optional[ast.Module]:
        path = self._module_source_path(module_name)
        if not path or not os.path.exists(path):
            return None
        stamp = self._file_stamp(path)
        cached = self._source_tree_cache.get(path)
        ...
        try:
            ...
            return None
        self._source_tree_cache[path] = {"stamp": stamp, "tree": tree}
        return tree

    def _find_source_target(self, module_name: str, chain: List[str]) -> Optional[ast.AST]:
        tree = self._module_ast(module_name)
        if tree is None:
            return None
        body: List[ast.stmt] = list(getattr(tree, "body", []) or [])
        ...
        for attr in (chain or []):
            ...
            if found is None:
                return None
            current = found
            body = list(getattr(found, "body", []) or []) if isinstance(found, ast.ClassDef) else []
        return current

    def _annotation_text(self, node: Optional[ast.AST]) -> str:
        if node is None:
            return ""
        try:
            return str(ast.unparse(node))
        except Exception:
            return getattr(node, "id", "") or getattr(node, "arg", "") or ""

    def _signature_from_ast(
        self,
        node: ast.AST,
        *,
        class_target: bool = False,
    ) -> Dict[str, Any]:
        target = node
        if class_target and isinstance(node, ast.ClassDef):
            init_node = None
            for child in node.body:
                if isinstance(child, (ast.FunctionDef, ast.AsyncFunctionDef)) and child.name == "__init__":
                    init_node = child
                    break
            target = init_node if init_node is not None else node

        profile = {
            "callable": bool(isinstance(target, (ast.FunctionDef, ast.AsyncFunctionDef)) or class_target),
            "class_target": bool(class_target),
            "async_callable": bool(isinstance(target, ast.AsyncFunctionDef)),
            "required_args": 0,
            "optional_args": 0,
            "kwonly_args": 0,
            "varargs": False,
        ...
        default_offset = max(0, len(positional) - len(defaults))
        ...
        for idx, arg in enumerate(positional):
            ...
            if arg.annotation is not None:
                rendered = f"{arg.arg}: {self._annotation_text(arg.annotation)}" + ("=..." if has_default else "")
            rendered_params.append(rendered)
            if arg.arg in payload_names:
                profile["accepts_payload"] = True

        if args.vararg is not None:
            profile["varargs"] = True
            rendered_params.append(f"*{args.vararg.arg}")
        elif args.kwonlyargs:
            rendered_params.append("*")

        for arg, default in zip(list(args.kwonlyargs or []), list(args.kw_defaults or [])):
            has_default = default is not None
            rendered = arg.arg
            if arg.annotation is not None:
                rendered = f"{arg.arg}: {self._annotation_text(arg.annotation)}"
            if has_default:
                rendered += "=..."
                optional += 1
            else:
                required += 1
            rendered_params.append(rendered)
            profile["kwonly_args"] = int(profile["kwonly_args"]) + 1
            if arg.arg in payload_names:
                profile["accepts_payload"] = True

        if args.kwarg is not None:
            profile["varkw"] = True
            rendered_params.append(f"**{args.kwarg.arg}")

        profile["required_args"] = int(required)
        profile["optional_args"] = int(optional)
        profile["signature_text"] = f"({', '.join(rendered_params)})"
        if target.returns is not None:
            profile["return_hint"] = self._annotation_text(target.returns)
        try:
    ...
    def _infer_contract_profile(
        ...
        if cache_key == tuple(self._contract_profile_cache.get("key", ()) or ()):
            ...
            return dict(cached) if isinstance(cached, dict) else {}
        target_node = self._find_source_target(module_name, chain)
        class_target = bool(isinstance(target_node, ast.ClassDef))
        ...
        if target_node is not None:
            profile.update(self._signature_from_ast(target_node, class_target=class_target))
    ...
    def _render_evolved_surfaces(
        ...
        alias_keys = {self._surface_alias_key(name): name for name in registry.keys()}
        for alias_name in self._legacy_surface_bindings():
            if alias_name in registry or alias_name in used_method_names:
                continue
            target_name = alias_keys.get(self._surface_alias_key(alias_name), "")
            used_method_names.add(alias_name)
            if target_name:
                alias_map[alias_name] = target_name
                method_blocks.append(
                    f"    def {alias_name}(self, payload: Any = None, **kwargs: Any) -> Dict[str, Any]:\n"
                    f"        return getattr(self, {target_name!r})(payload=payload, **kwargs)\n"
                )
                continue
            registry[alias_name] = {
                ...
                "effect_phrases": ["legacy evolved-surface binding preserved across promotion"],
            }
            alias_map[alias_name] = ""
            method_blocks.append(
                f"    def {alias_name}(self, payload: Any = None, **kwargs: Any) -> Dict[str, Any]:\n"
                f"        meta = dict(self._registry.get({alias_name!r}, {{}}) or {{}})\n"
                f"        return self._reflect_surface({alias_name!r}, meta, payload=payload, **kwargs)\n"
            )
        ...
        manifest = {
            ...
            "compatibility_alias_count": len(alias_map),
