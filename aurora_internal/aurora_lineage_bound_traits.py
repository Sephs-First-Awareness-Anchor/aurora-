...
class LineageBoundTraitRegistry:
    ...
    def _derive_viability_metadata(self, spec: LineageTraitSpec) -> Dict[str, Any]:
        stage_count = max(1, len(tuple(spec.stages or ())))
        binding_count = len(tuple(spec.bindings or ()))
        patch_count = len(tuple(spec.runtime_patch_targets or ()))
        viable_band_alignment = max(
            0.15,
            min(0.95, 0.42 + (0.07 * stage_count) + (0.04 * binding_count) + (0.03 * patch_count)),
        )
        return {
            "ontological_status": "emerging" if stage_count > 0 else "latent",
            "viable_band_alignment": round(float(viable_band_alignment), 4),
    ...
    def materialize(self, spec: LineageTraitSpec) -> Dict[str, str]:
        ...
        viability_metadata = self._derive_viability_metadata(spec)
        ...
        activation_manifest = {
            ...
            "runtime_contract": {
                ...
                "bindings": [binding.to_dict() for binding in spec.bindings],
                "viability_metadata": viability_metadata,
            },
