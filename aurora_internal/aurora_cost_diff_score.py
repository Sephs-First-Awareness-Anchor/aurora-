...
def _derive_operator_scales() -> Dict[Constraint, float]:
    ...
    Earlier builds compressed X/T/N by arbitrary decade tiers. That made the
    energy layer behave almost detached from the cheap existence/time substrate
    that actually lets energy mean anything over real ticks. The scale now stays
    cost-derived: every layer contributes in proportion to its shift cost
    relative to Agency, preserving A dominance without making X/T/N silent.
    """
    max_coeff = max(
        REGISTRY.cost(c).shift_cost_coeff
        for c in (Constraint.X, Constraint.T, Constraint.N, Constraint.B, Constraint.A)
...
OPPOSED_OPERATOR_SCALES: Dict[Constraint, float] = _derive_operator_scales()
...
TICK_PARTICIPATION_WEIGHTS: Dict[Constraint, float] = {
    ...
    Constraint.N: 1.5,    # balanced between existence and time support
...
def _xt_energy_support(abs_deltas: Dict[Constraint, float]) -> float:
    """
    Energy is only meaningful when it is grounded by existence and time.

    N remains its own signed redistribution channel, but its pressure should not
    be almost independent from the X/T layers that supply admissibility and tick
    persistence. This helper produces a small support term from X and T drift so
    N can participate when the substrate is moving even if N:D itself is quiet.
    """
    x = max(0.0, float(abs_deltas.get(Constraint.X, 0.0)))
    t = max(0.0, float(abs_deltas.get(Constraint.T, 0.0)))
    if x <= 0.0 and t <= 0.0:
        return 0.0
    # T carries persistence, X carries admissibility; keep both present.
...
def per_operator_pressure(snapshot: Optional[DifferenceSnapshot]) -> Dict[Constraint, float]:
    ...
    energy_support = _xt_energy_support(abs_deltas)
    ...
    for c in ordered:
        ...
        if c is Constraint.N:
            # N is the conservation channel, but it must stay coupled to the
            # X/T substrate that supplies admissibility and tick persistence.
            own = (0.60 * own) + (0.40 * energy_support)
