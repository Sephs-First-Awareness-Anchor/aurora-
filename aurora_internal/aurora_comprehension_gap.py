...
class GapType(Enum):
    ...
    NONSENSE      = "nonsense"       # Input was incorrect audio parsing / garbage
...
class ComprehensionGap:
    ...
    pending: bool = True
    axis_tension: Dict[str, float] = field(default_factory=dict)
        ...
        if oets:
            ...
            if web:
                if not web.has_node(word):
                    web.add_node(word, role, valence, lineage='reasoning_anchor')
                
                node = web.get_node(word)
                if node:
                    # Store the resolution logic: Link the word to the specific axis tension
                    tension = getattr(gap, 'axis_tension', {})
                    dominant = max(tension, key=lambda k: abs(tension[k])) if tension else "X"
                    
                    node.add_definition(answer, source='structural_reasoning', confidence=0.98)
                    
                    # Create reasoning logic node and link it
                    if not web.has_node("reasoning_logic"):
                        web.add_node("reasoning_logic", "noun", 0.0)
                        
                    web.add_relation(
                        source=word,
                        target="reasoning_logic",
                        relation_type=RelationType.CAUSES, 
                        strength=0.95,
                        knowledge_source=f"axis_{dominant}_alignment"
                    )
                updated.append('oets')

        if composer:
            # Let her absorb the reasoning structure itself
            composer.absorb(answer[:200], tone='precise')
            # Link the source text to the reasoning logic pattern
            composer.absorb(gap.source_text, tone='neutral')
            updated.append('composer')

        note = f"Reasoning for '{word}' applied to {max(getattr(gap, 'axis_tension', {}), key=lambda k: abs(gap.axis_tension[k]), default='X')}-axis."
        return updated, note
...
class ComprehensionGapSystem:
    ...
    def process(self, user_text: str, systems: Dict[str, Any],
        ...
        # 100% Scripted-Free Nonsense Handling: Flag the error and return None 
        # to allow the generative pipeline (ConstraintEmitter) to respond.
        if self._looks_like_nonsense(user_text):
            if working_memory:
                # Flag recent grounding as unreliable
                working_memory.last_intake_was_nonsense = True
            return None
    ...
    def _build_acknowledgment(self, resolved: ResolvedGap) -> str:
        ...
        """
        gap = resolved.gap
        _raw = resolved.answer_text
        answer = _raw if len(_raw) <= 90 else _raw[:90].rsplit(' ', 1)[0]
        gtype = gap.gap_type
    ...
    _NONSENSE_MARKERS = (
        "nonsense", "that's nonsense", "that was nonsense", "ignore that",
        "incorrect parsing", "bad audio", "didn't say that", "wasn't saying that",
    )
    ...
    def _looks_like_nonsense(self, user_text: str) -> bool:
        """Return True if the user is flagging the last turn as nonsense/incorrect parsing."""
        low = user_text.lower().strip()
        return any(m in low for m in self._NONSENSE_MARKERS)
