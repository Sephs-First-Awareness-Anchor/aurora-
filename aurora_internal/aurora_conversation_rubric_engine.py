...
def _generic_response_penalty(text: str) -> float:
    ...
    if not text:
        return 0.0
    sentence_count = max(1, len([s for s in re.split(r"[.!?]+", str(text)) if s.strip()]))
    token_count = max(1, len(re.findall(r"[a-z0-9]{3,}", str(text).lower())))
    specificity = min(1.0, (len(terms) / max(4.0, float(token_count))) + min(0.35, sentence_count * 0.06))
    return _clamp(1.0 - specificity)
