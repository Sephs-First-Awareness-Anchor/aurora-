        ...
        for web_path in load_candidates:
            try:
                ...
                for word, ndata in nodes_data.items():
                    ...
                    node.noncomp_id = ndata.get("noncomp_id", None)
...
@dataclass
class MemoryEntry:
    """A single memorable interaction — stored entirely in constraint-native form.

    No English text is persisted here.  The FGAE is the only translation layer;
    everything below it works in constraint coordinates.

    input_nc  — constraint projection of the user's turn (FGAE noncomp output):
                 axis, dimension, nc_name, anchor, worth_score, frame, etc.
    response_nc — constraint axes snapshot of Aurora's response:
                 runtime_axes, dominant, confidence, nc_name.

    Legacy fields user_said / aurora_said are accepted at load time for
    backward-compat with old files but are never written going forward.
    """
    timestamp: float
    input_nc: dict = field(default_factory=dict)    # FGAE constraint projection of input
    response_nc: dict = field(default_factory=dict) # constraint axes snapshot of response
    topic: str = ""           # constraint category label
    emotional_tone: str = "neutral"
    importance: float = 0.5
    session_id: str = ""
    trace_id: str = ""
    # Legacy compat — never written, silently accepted on load
    user_said: str = ""
    aurora_said: str = ""
...
class ConversationMemory:
    ...
    def record_exchange(self, user_text: str = "", aurora_text: str = "",
                        ...
                        response_nc: dict = None, input_nc: dict = None):
        """Record a memorable exchange — constraint-native only.

        Neither user_text nor aurora_text is stored.  The caller supplies:
          input_nc   — FGAE constraint projection of the user's turn.
          response_nc — constraint axes snapshot of Aurora's response.
        Human-readable text (user_text, aurora_text) is accepted for API
        compatibility but immediately discarded — the FGAE is the only
        translation layer.
        """
        self._prune_surface_window()
        ...
        entry = MemoryEntry(
            timestamp=time.time(),
            input_nc=dict(input_nc or {}),      # constraint projection — no English
    ...
    def recall_about(self, topic_or_person: str) -> List[str]:
        ...
        # Check memorable exchanges — match on constraint coordinates, not English text
        for entry in self.entries:
            inc = entry.input_nc if isinstance(entry.input_nc, dict) else {}
            rnc = entry.response_nc if isinstance(entry.response_nc, dict) else {}
            # Match key against: anchor, nc_name, topic, axis label
            matchable = " ".join(filter(None, [
                str(inc.get("anchor") or ""),
                str(inc.get("nc_name") or ""),
                str(inc.get("constraint") or ""),
                str(inc.get("dimension") or ""),
                str(entry.topic or ""),
                str(rnc.get("dominant") or ""),
                str(rnc.get("nc") or ""),
            ])).lower()
            if key in matchable:
                axes = rnc.get("axes") or {}
                axes_str = " ".join(
                    f"{ax}={round(float(v),2)}"
                    for ax, v in sorted(axes.items())
                    if float(v or 0) > 0.1
                )
                results.append(
                    f"[{entry.emotional_tone}] "
                    f"input_nc:{inc.get('nc_name','?')} anchor:{inc.get('anchor','?')} "
                    f"topic:{entry.topic} dominant:{rnc.get('dominant','?')}"
                    + (f" axes:[{axes_str}]" if axes_str else "")
                )
            ...
            "entries": [
                {
                    ...
                    "user_said": e.user_said,   # legacy compat
                    "aurora_said": e.aurora_said,  # legacy compat
                    "topic": e.topic,
...
class EnhancedStatePersistence:
    ...
    def load_all(self, systems: Dict[str, Any]) -> Dict[str, bool]:
        ...
        # Load OETS web — skipped for surface stratum.
        # Surface holds short conversational continuity only; the deep semantic
        # web (26MB+ on disk, ~400MB in memory) lives in the subsurface.
        # Surface gets a minimal identity-seeded stub instead.
        perception = systems.get('perception')
        _stratum = str(systems.get('stratum_role', 'full') or 'full')
        if perception and getattr(perception, 'oets', None):
            if _stratum == 'surface':
