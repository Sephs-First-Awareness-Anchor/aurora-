...
class CrystalFacet:
    ...
    def observe(self, vec: List[float], session_id: str,
        ...
        admission_weight = _clamp01(sim / max(threshold, 1e-9)) if node is not None else 0.0
        if node is not None:
            ...
            node.confidence = _ema(node.confidence, confidence_hint * max(0.25, admission_weight))
            node.compute_fitness()
            if sim >= threshold or admission_weight >= 0.6:
                return node.node_id
        ...
        if len(self._nodes) < self.candidate_cap:
            ...
            node.usage_count   = 1
            node.session_count = 1
            node.confidence    = confidence_hint * max(0.35, admission_weight or 1.0)
            node.compute_fitness()
    ...
    def tick_advanced_promotion(self) -> List[str]:
        ...
        for node in self._nodes.values():
            promotion_score = (
                0.50 * _clamp01(node.fitness)
                + 0.30 * _clamp01(node.usage_count / float(max(1, CONCEPT_PROMOTION_USAGE * 2)))
                + 0.20 * _clamp01(len(node.cross_modal_links) / 3.0)
            )
            if node.stage == "concept" and promotion_score >= 0.72:
                node.stage       = "promoted"
                node.generation += 1
                out.append(node.node_id)
                logger.debug("[SensoryCrystal/%s/%s] concept->promoted %s  score=%.2f fitness=%.2f",
                             self.domain, self.facet, node.node_id[:8], promotion_score, node.fitness)
                ...
                if getattr(self, "_evolution_hook", None) is not None:
                    try:
                        axis_map = {
...
class SemanticCrystalNode:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "SemanticCrystalNode":
        ...
        lane_to_pair = {lane: pair for pair, lane in LANE_LABEL.items()}
        if ("audio_facet" not in payload or "visual_facet" not in payload) and payload.get("lane") in lane_to_pair:
            audio_facet, visual_facet = lane_to_pair[str(payload["lane"])]
            payload.setdefault("audio_facet", audio_facet)
            payload.setdefault("visual_facet", visual_facet)
        return cls(**payload)
...
class AuroraSensoryCrystal:
    ...
    def register_evolution_hook(self, hook: Callable[[Dict[str, Any]], None]) -> None:
        ...
        # Propagate to individual CrystalFacet instances so tick_advanced_promotion fires it
        for facet in list(self._audio.values()) + list(self._visual.values()):
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        state["lineage_signature"] = cp.weighted_signature() if hasattr(cp, "weighted_signature") else "XTNBA"
        ...
        try:
            ...
            _SENSORY_ABILITY_PROFILES: Dict[str, Any] = {
                ...
                "B:SENSORY_CLUSTER": AbilityProfile(
                    ...
                    notes        = "Observations cluster into SensoryNode prototypes (sensory.cluster, gen=2, B-axis)",
                ),
                "A:SENSORY_PROMOTE": AbilityProfile(
                    ...
                    notes        = "primitive→concept→promoted node progression (sensory.promote, gen=2, A-axis)",
                ),
