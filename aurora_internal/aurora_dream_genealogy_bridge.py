...
class DreamGenealogyBridge:
    ...
    def generate_expression_hints(
        ...
        for dim in RUBRIC_DIMENSIONS:
            score = float(summary.mean_scores.get(dim, 0.5) or 0.5)
            direction = "reinforce" if score >= 0.5 else "attenuate"
            strength = _clamp(abs(score - 0.5) * 2.0, 0.1, 0.8)
            if direction == "reinforce":
                description = (
                    f"Dream performance in {dim} "
                    f"(score={score:.2f}) supports the current pattern"
                )
            else:
                description = (
                    f"Dream performance in {dim} "
                    f"(score={score:.2f}) suggests softening the current pattern"
                )
            hints.append(ExpressionWritebackHint(
                hint_id=_generate_id("ewh_r" if direction == "reinforce" else "ewh_a"),
                dimension=dim,
                direction=direction,
                strength=strength,
    ...
    def generate_learner_observations(
        ...
        # Generate observations for every dimension with graded intensity.
        for dim, score in sorted(
            summary.mean_scores.items(), key=lambda kv: kv[1]
        ):
            obs = {
                "avatar_engaged": score >= 0.5,
                "conversation_deepened": score >= 0.55,
                "connection_felt_stronger": score >= 0.60,
                "tension_arose": score < 0.45,
                "flow_maintained": score >= 0.50,
                "context_type": f"dream_{dim}",
                "understanding_text": self._generate_understanding(dim, score),
                "graded_strength": _clamp(abs(score - 0.5) * 2.0, 0.05, 1.0),
            }
