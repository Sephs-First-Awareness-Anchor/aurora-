...
class IVMLattice:
    ...
    def __init__(self, contract: FoundationalContract, max_nodes: int = 100000):
        ...
        # 31-field power-set observer (read-only)
        self.field_map = None
    ...
    def admit(self,
        ...
        mode_val = profile.mode if hasattr(profile, 'mode') else profile
        phase_vector = self.vertices.get_phase_vector(mode_val)
        position = IVMCoordinate(
            mode=mode_val,
            phases=phase_vector,
            scale=scale,
...
class ContradictionLedger:
    ...
    STATE_PATH = "aurora_state/contradiction_ledger.json"
...
_AURORA_NATIVE_STRATEGIES = {'IVMLattice.develop_agency': {'ability_hits': 0,
                               'alignment_gap': 0.0,
                               ...
                               'effect_phrases': ['would extend agency pressure handling',
                                                  ...
                                                  'by aurora_ivm.IVMLattice'],
                               'genealogy_pressure': 0.0,
                               'inheritance_breach_count': 0,
                               'kind': 'latent',
                               'link_hits': 0,
                               'module': 'aurora_ivm',
                               'op_id': 'latent.aurora_ivm.IVMLattice.develop_agency',
                               'origin_activity': 0,
...
def develop_agency(payload=None, **kwargs):
    ...
    if engine is None:
        return {
            'available': False, 'reason': 'evolved_surface_engine_unavailable', 'op_id': 'latent.aurora_ivm.IVMLattice.develop_agency', 'kind': 'latent'
        }
    return getattr(engine, 'latent_aurora_ivm_ivmlattice_develop_agency')(payload=payload, **kwargs)

_aurora_existing_binding = _aurora_get_target(['IVMLattice'])
if _aurora_existing_binding is not None:
    _aurora_existing_attr = getattr(_aurora_existing_binding, 'develop_agency', None)
    if _aurora_existing_attr is None or getattr(_aurora_existing_attr, '_aurora_latent_binding_target', '') == 'IVMLattice.develop_agency':
        _aurora_bind_owner_attribute(['IVMLattice'], 'develop_agency', _aurora_make_latent_binding('develop_agency', 'IVMLattice.develop_agency'))
        _AURORA_NATIVE_EVOLVED_LAST['IVMLattice.develop_agency'] = {'latent_binding_active': True}

AURORA_NATIVE_EVOLVED_EXPORTS = {'latent.aurora_ivm.IVMLattice.develop_agency': 'develop_agency'}
AURORA_NATIVE_EVOLUTION_OVERRIDES = {'latent.aurora_ivm.IVMLattice.develop_agency': {'export': 'develop_agency',
                                                 'mode': 'latent_binding',
                                                 'target': 'IVMLattice.develop_agency'}}
