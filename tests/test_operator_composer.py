...
def test_builtin_self_verification_all_pass():
    results = verify_operator_composer("/home/user/aurora-")
...
def test_rediscovery_self_test_never_touches_real_production_state():
    ...
    import os
    real_path = "/home/user/aurora-/aurora_state/operation_descriptors.json"
    before = os.path.getmtime(real_path)
    verify_operator_composer("/home/user/aurora-")
...
def test_persist_never_duplicates_existing_op_ids(tmp_path):
    ...
    # composing the exact same tick/parents again mints the SAME op_id
    # (hash of op_a:op_b:tick) -- persisting again must not duplicate it.
