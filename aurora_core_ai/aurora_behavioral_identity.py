...
class BehavioralFacet:
    ...
    def mutate(self, pressure: float = 1.0) -> float:
        """Mutate this facet. Returns magnitude of change."""
        delta = random.gauss(0, self.evolution_rate * pressure)
...
class BehavioralCrystal:
    ...
    def evolve(self, pressure: float = 1.0) -> Dict[str, float]:
        """Evolve all facets. Returns mutations."""
        self.generation += 1
        mutations = {}
        for name, facet in self.facets.items():
            delta = facet.mutate(pressure)
            if delta > 0.005:
...
class BehavioralIdentityEngine:
    ...
    def _constraint_axes(self) -> Dict[str, float]:
        ...
        return {
            ...
            "N": _clamp(abs(sum(t.drift_from_base() for t in self.traits.values())) / max(len(self.traits), 1) + 0.18),
    ...
    def _pressure_axes(self) -> Dict[str, float]:
        drift = _clamp(abs(sum(t.drift_from_base() for t in self.traits.values())) / max(len(self.traits), 1))
        pressure = {axis: drift * 0.35 for axis in ("X", "T", "N", "B", "A")}
        if self._last_constraint_axis:
            axis = {'existence': 'X', 'temporal': 'T', 'energy': 'N', 'boundary': 'B', 'agency': 'A'}.get(str(self._last_constraint_axis), "")
            if axis in pressure:
                pressure[axis] = _clamp(pressure[axis] + 0.45)
        pressure["B"] = _clamp(pressure["B"] + min(0.25, len(self.dna.genome.identity_anchors) / 20.0))
        return pressure

    def constraint_profile(self):
        return build_constraint_profile(
            unit_id="behavioral_identity_engine",
            unit_kind="identity_engine",
            operational_role="dna_traits_anchors_behavioral_selfhood",
            genealogy="XTNBBA",
