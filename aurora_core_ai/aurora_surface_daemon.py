    ...
    def _camera_capture_loop() -> None:
        ...
        while not shutdown:
            ...
            if not camera_is_enabled:
                ...
                continue
            try:
                if integration is not None and hasattr(integration, "see"):
                    _desc, _visual = integration.see()
                    if _visual:
                        empty_reads = 0
                    else:
                        empty_reads += 1
                        if empty_reads in (1, 5, 15):
                            _log(f"Surface camera read empty ({_desc})")
                elif perception is not None and hasattr(perception, "see"):
                    _desc, _visual = perception.see()
                    if _visual:
                        empty_reads = 0
                    else:
                        empty_reads += 1
                    ...
                    if visual is not None and hasattr(hardware, "process_visual"):
                        hardware.process_visual(visual, None)
                        empty_reads = 0
                    else:
                        empty_reads += 1
                        if empty_reads in (1, 5, 15):
