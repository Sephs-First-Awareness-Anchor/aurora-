...
class Aurora625PressureMap:
    ...
    def build(self, ops: List[Dict[str, Any]], latent_ops: Optional[List[Dict[str, Any]]] = None) -> None:
        ...
        print(f"[625MAP] Profiles built. Occupied: {occupied}/625. Empty: {625 - occupied}/625.")
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            for slot, entry in data.get("slots", {}).items():
                ...
                p.hybrid_w = prof.get("lineage", {}).get("hybrid", 0.0)
                p.offspring_w = prof.get("lineage", {}).get("offspring", 0.0)
                p.ancestor_w = prof.get("lineage", {}).get("ancestor", 0.0)
                p.transitional_w = prof.get("lineage", {}).get("transitional", 0.0)
