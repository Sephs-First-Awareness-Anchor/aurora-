...
class ConsciousnessEngine:
    ...
    def _pressure_axes(self) -> Dict[str, float]:
        alignment = float(getattr(self.entropy.state, "alignment", 0.5) or 0.5)
        return {
            "X": max(0.0, min(1.0, 1.0 - float(getattr(self.entropy.state, "coherence", 0.5) or 0.5))),
            "T": max(0.0, min(1.0, float(getattr(self.entropy.state, "stagnation_score", 0.0) or 0.0))),
            "N": max(0.0, min(1.0, float(getattr(self.entropy.state, "vitality_pressure", 0.0) or 0.0))),
            "B": max(0.0, min(1.0, abs(alignment - 0.5) * 2.0)),
            "A": max(0.0, min(1.0, len(self._pending_warps) * 0.20 + (0.12 if self._simulation_engine is not None else 0.0))),
        }

    def constraint_profile(self):
        return build_constraint_profile(
            unit_id="consciousness_engine",
            unit_kind="consciousness_orchestrator",
            operational_role="entropy_dce_dpme_assembly",
            genealogy="XTNBAA",
