...
Everything runs in ONE Python process — no subprocess daemons.  Three separate
heavyweight Python processes was the root cause of Android OOM kills (Signal 9).
...
Say "aurora <anything>" and she responds in-process.  When idle she runs her
curiosity/autonomy cycles in a background thread.
...
def _extract_response(result: Any) -> str:
    """Extract human-readable response text from the pipeline result."""
    if not isinstance(result, dict):
        return str(result or "").strip()
    # Primary path: resp_A.content (standard dual-question pipeline)
    resp_a = result.get("resp_A")
    if resp_a is not None:
        content = getattr(resp_a, "content", None)
        if isinstance(content, list):
            # Anthropic API content-block list — extract text blocks
            parts = [
...
# ---------------------------------------------------------------------------
def _make_on_utterance(systems_holder: Dict[str, Any], speak_fn) -> Any:
    """
    Returns a callback that processes a user utterance directly in-process.
    systems_holder["systems"] is populated after boot_aurora() completes;
    utterances received before that are dropped with a log warning.
    """
    ...
    def _on_utterance(text: str) -> None:
        ...
        try:
            ...
            try:
                ...
                # Function signature may not accept all kwargs — try minimal call
                try:
                    result = process_external_user_turn(systems, text)
                except Exception as e2:
                    _log(f"  [ERROR] Turn processing failed: {e2}")
...
def _boot_hardware_io(on_utterance) -> Optional[Any]:
    """Import and start HardwareIO.  Returns instance or None on failure."""
    try:
...
def _start_file_ptt_watcher(on_utterance) -> threading.Thread:
    """
    Polls voice_trigger.json for manual text injection.
...
# Status display
# ---------------------------------------------------------------------------
def _print_status(systems_holder: Dict[str, Any]) -> None:
    systems = systems_holder.get("systems")
...
def main() -> None:
    ...
    # Shared mutable holder so on_utterance can reference systems after boot
    systems_holder: Dict[str, Any] = {"systems": None}

    # ── 1. TTS first — boot messages can be spoken immediately ────────────
    speak_fn = _make_speak_fn()

    # ── 2. Set up turn callback (reads systems_holder lazily after boot) ──
    on_utterance = _make_on_utterance(systems_holder, speak_fn)

    # ── 3. HardwareIO (wake-word listener + TTS) ──────────────────────────
    _log("[BOOT] Starting hardware I/O...")
    hw = _boot_hardware_io(on_utterance)

    # ── 4. File PTT watcher — always-on, no hardware required ─────────────
    _log("[BOOT] File PTT watcher active.")
    _log(f"  → echo '{{\"trigger\":true,\"text\":\"hello\"}}' > {_VOICE_TRIGGER}")
    _start_file_ptt_watcher(on_utterance)

    # ── 5. Proactive message poller ────────────────────────────────────────
    _start_proactive_poller(speak_fn)

    # ── 6. Boot the full Aurora stack (one boot call — no subprocesses) ────
    ...
    # ── 7. CuriosityEngine — idle autonomy / dreaming in a bg thread ──────
    _log("[BOOT] Starting curiosity / autonomy engine...")
    _start_curiosity_engine(systems)

    # ── 8. Periodic state saves ────────────────────────────────────────────
