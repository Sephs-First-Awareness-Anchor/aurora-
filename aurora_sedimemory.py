...
class MemoryEvent:
    ...
    def _derive_lineage_signature(self) -> str:
        ax = self._axis_weights()
        ordered = [a for a in ("X","T","N","B","A") if float(ax.get(a, 0.0) or 0.0) > 0.0]
        base = "".join(ordered) or "X"
        if ordered:
            dominant = max(ordered, key=lambda a: float(ax.get(a, 0.0) or 0.0))
            if len(ordered) == 5 and float(ax.get(dominant, 0.0) or 0.0) >= 0.30:
                return base + dominant
        return base

    def _derive_memory_contract(self) -> Dict[str, Any]:
        ax = self._axis_weights()
        dominant = max(ax, key=ax.__getitem__) if ax else "X"
        decay = min(ax, key=ax.__getitem__) if ax else "T"
        return {
            "lineage_signature": self.lineage_signature,
            "decay_axis": decay,
            "transformation_axis": dominant,
            ...
            "weighted_form": {
                "dominant_axis": dominant,
                "signature": self.lineage_signature,
            },
        }
...
class SedimentBasin:
    ...
    # CBU profile — attached on construction by SedimentColumn._build_basins()
...
class SedimentColumn:
    ...
    def _build_basins(self) -> None:
        # CBU depth profile mapping (CBU directive Step 7 + §A1)
        _depth_cfg = {
            "X": ("XTNBA", {"A_weight": 1.0}),                     # surface, operation-heavy
            "T": ("XTNB",  {"T_weight": 0.8}),                     # layer 1
            "N": ("XTNB",  {"N_weight": 0.8}),                     # layer 2
            "B": ("XTN",   {"X_weight": 0.7, "T_weight": 0.6}),    # layer 3-4
            "A": ("XT",    {"X_weight": 1.0, "T_weight": 0.8}),    # deep archive
        }
        for constraint in Constraint.all():
            for dimension in NonCompDimension:
...
class SediMemory:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        s["lineage_signature"] = "".join(ax for ax in ("X","T","N","B","A") if getattr(cv, ax) > 0.01)
    ...
    def constraint_profile(self) -> _ConstraintVector:
        ...
        return _ConstraintVector(
            X=max(1e-9, float(ax.get("X", 0.22))),
